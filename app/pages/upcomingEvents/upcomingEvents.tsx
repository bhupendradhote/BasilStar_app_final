  ...
  const fetchEarningsEvents = async () => {
    try {
      ...
      const mapped = data.map((item: any) => ({
        ...
        type: "Earnings",
        title: item.symbol,
        description: `EPS Actual: ${item.epsActual ?? "N/A"} | EPS Estimated: ${
          item.epsEstimated ?? "N/A"
        } | Revenue Actual: ${item.revenueActual ?? "N/A"} | Revenue Estimated: ${
          item.revenueEstimated ?? "N/A"
        }`,
      }));
